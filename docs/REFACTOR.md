...
- `python -m anima` 默认桌面模式仍由 `anima/desktop/app.py`(pywebview)兜底,不受影响。(**Phase 4 已整删 pywebview**,改为 `_launch_web` 直接开浏览器。)
...
### Phase 4 剩余(可选,非阻塞)
- ✅ **可选 extras 瘦身已完成**:base 仅 8 个依赖(litellm/aiohttp/chromadb…);`pyzmq/msgpack/zeroconf/paramiko→[network]`、voice→[tts]/[stt];`pip install anima[all]` 装全。缺 extra 时特性优雅禁用(`_init_network` 守卫 + spawn 友好提示),不崩。**决定不拆物理多包**(依赖箭头是 robotics→core,薄壳反转 churn 高收益低)。
- ✅ **pywebview 桌面壳整删**:删 `desktop/app.py` + `[desktop]` extra;`python -m anima` 改为 `_launch_web`(单例锁 + 开浏览器)→ `main_entry`。ANIMA 是网页应用,**只留 eva-ui 一个前端**;VRM 头像静态资源仍经 `/desktop/static/` 提供(eva-ui 加载 `Flare.vrm`),单例锁 `desktop/singleton.py` 保留。
...
- **启动**:`python -m anima` → `anima/__main__.py:_launch_web`(单例锁 + 开浏览器)→ `main_entry()` → `anima/main.py:run()` 分阶段 init:core→robotics→llm→heartbeat→network→cognitive→dashboard→wire_callbacks。约 60 处全局 `set_xxx()` 注入,无 DI 容器。(pywebview 已移除;eva-ui 是唯一前端。)
