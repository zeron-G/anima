...
### Phase 4 剩余(可选,非阻塞)
- ✅ **可选 extras 瘦身已完成**:base 仅 8 个依赖(litellm/aiohttp/chromadb…);`pyzmq/msgpack/zeroconf/paramiko→[network]`、`pywebview→[desktop]`、voice→[tts]/[stt];`pip install anima[all]` 装全。缺 extra 时特性优雅禁用(`_init_network` 守卫 + desktop/spawn 友好提示),不崩。**决定不拆物理多包**(依赖箭头是 robotics→core,薄壳反转 churn 高收益低)。
