<!-- 讨论稿 v0.2 — 工作坊综合(v0.1) + 与主人敲定的锁定决策，2026-07 -->
<!-- 状态：核心架构已锁定(见下"锁定的决策")；下方 §1-§9 为设计依据全文；两个 live P0 见 §7 必须先关。 -->

# ANIMA 分布式架构（v0.2）

## 锁定的决策（2026-07 与主人敲定 — 权威，凡与下方 §1-§9 冲突以此为准）

分布式取向 = **Architecture C**，进一步锁定为**纯对等、永远无主、每节点全量本地副本**：

1. **存储拓扑：纯 P2P 本地优先，每节点全量副本，永远无主。** 每节点持完整本地 Postgres（离线独立跑），N 路反熵最终一致同步（origin-tagged）。加服务器 = 直接多一个对等副本。→ 目标态**推翻旧"单一 Neon / 数据先行上云"决定**（[[project_postgres_migration]]）；Phase 1-2 仍可用单 Neon 过渡，Phase 3 翻转。
2. **PiDog = 全量副本 + 量化索引。** 不做特殊/部分节点，与其他节点无差别对等；量化/精简 HNSW 塞进 RPi。⚠ watch-item：靠冷记忆衰减剪枝控住向量体积（Phase 3）。
