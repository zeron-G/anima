...
#### P3e-1 灵魂快照+恢复（durability 核心）✅ 已完成（commit 670ae73）
...
#### P3e-2 static_knowledge global 云权威（tiered）✅ 已完成（commit b5e6b74）
- `static_store.py`：`StaticKnowledgeStore` 加 `long_term` 句柄，`self._tiered = cloud is not working`。tiered 下 `_store_for(scope)`：global→cloud、node:<id>→working。`query` union(本地[global,node] + 云[global])→`_merge`。`upsert` global→cloud、node→working。`delete` global→双层墓碑。`main.py` 传 `long_term=getattr(memory_store,'long_term',None)`。非分层(long_term None)零改动=单 store 原行为。
- **对抗式复审 workflow wf_2196f437 抓 5 confirmed（2 HIGH 2 MED 1 LOW，两个根因）全修**：① 跨 store 比 `version` 挑赢家无意义（working/cloud 独立版本计数器，tiered 下 pg_sync 关）→ 陈旧本地 global 高版本会盖新云值 → `_merge` 改 **cloud 优先（first-wins，云权威）**，不比版本 ② **云 store 的 failover DSN 默认=LOCAL_DATABASE_URL（=working 同库）** → 离线 global 写静默落到本地被搁浅/离线 global 删墓碑打错库 → `pg_db`/`pg_store.create` 加 `allow_local_failover`，`store.py` 云层 `allow_local_failover=False`（云 down 显式抛，不静默污染本地）；`static_store.upsert` 云 down→本地 fallback 不丢不崩、`delete` 双层 best-effort `cloud_ok or local_ok`。docker 双 PG `verify_p3e2.py` 11 场景全过(路由/union/**云权威胜高版本本地**/双墓碑/云 down 读降级/**离线 upsert 本地 fallback**/离线 delete 本地墓碑/非分层不变)。62 单测无回归。
- **v1 取舍**：离线 global 写落本地为 fallback、云恢复后不自动回推（cloud-wins merge 会盖过它——除非重写云）；mesh-time 再补自动回推。
