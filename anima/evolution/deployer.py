...
Safely merges verified evolution branches into private,
pushes to remote, triggers hot-reload, and auto-rollbacks on failure.
"""
...
VERIFY_TIMEOUT_S = 30  # Seconds to wait after deploy before verifying
...
class Deployer:
    ...
    def deploy(self, proposal: Proposal, worktree_branch: str) -> bool:
        ...
        try:
            ...
            if push_result.returncode != 0:
                log.warning("Push failed (will retry later): %s", push_result.stderr)
            ...
            self._memory.record_success(
                ...
                lesson=f"Successfully deployed: {proposal.solution[:200]}",
            )
    ...
    def rollback(self, proposal: Proposal, reason: str) -> bool:
        """Revert the last commit (assumed to be the evolution merge)."""
        try:
            ...
            self._memory.record_failure(
                ...
                lesson=f"Deploy verification failed: {reason}",
            )
    ...
    async def verify_deployment(self) -> tuple[bool, str]:
        ...
        log.info("Verifying deployment (waiting %ds)...", VERIFY_TIMEOUT_S)
        await asyncio.sleep(VERIFY_TIMEOUT_S)

        # Check log for errors since deployment
